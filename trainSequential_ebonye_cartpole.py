...
def train_step(
    model, xs, ys, optimizer, loss_func, i, args, numtrainingsteps, b=0.5, g=9.81
):
    """
...
def load_dataset_chunk(pickle_folder, start_idx, end_idx):
    ...
    with tqdm(
        ...
        desc=f"Loading files {start_idx}-{end_idx}",
        leave=False,
    ) as load_pbar:
...
def evaluate_model(model, id_data, loss_func):
    ...
    id_data = TensorDataset(
        id_data[0],
        id_data[1],
        torch.tensor(id_data[2]),
        torch.tensor(id_data[3]),
        torch.tensor(id_data[4]),
    )

    id_loader = DataLoader(
        id_data, batch_size=64, shuffle=False
...
def train(model, args):
    ...
    optimizer = torch.optim.AdamW(
        model.parameters(), lr=args.training.learning_rate, weight_decay=1e-4
    )  # 1e-2
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        chunk_to_resume = 0
        with tqdm(
            total=num_chunks - chunk_to_resume, desc="Chunk Progress"
        ) as chunk_pbar:  ###ebonye 150
            for chunk_idx in range(chunk_to_resume, num_chunks):
                if args.use_chunk == 1:
                    dataset, dataset_full, cartmasses, polemasses, polelengths = (
                        load_dataset_full(fullpicklepath)
                    )
                else:
                    ...
                    dataset, dataset_full, cartmasses, polemasses, polelengths = (
                        load_dataset_chunk(fullpicklepath, start_idx, end_idx)
                    )
                ...
                with tqdm(
                    ...
                    desc=f"Training Chunk {chunk_idx + 1}/{num_chunks}",
                ) as pbar:
                    ...
                    for xs, ys, cartmass, polemass, polelength in dataloader:
                        ...
                        loss, output, gradnorm, oldgradnorm = train_step(
                            model,
                            xs,
                            ys,
                            optimizer,
                        ...
                        print(
                            f"Epoch {epoch + 1}/{num_epochs}, Step {current_step}, Loss: {loss}, Current LR: {optimizer.param_groups[0]['lr']}, Old Grad Norm: {oldgradnorm}"
                        )
                        # import pdb; pdb.set_trace()
                        ...
                        if (
                            current_step % args.wandb.log_every_steps == 0
                        ...
                        if (
                            current_step % args.training.save_every_steps == 0
                            and not args.test_run
                        ):
                            training_state = {
                            ...
                            checkpoint_path = os.path.join(
                                args.out_dir,
                                f"checkpoint_epoch{epoch + 1}_step{current_step}.pt",
                            )
                            ...
                            print(
                                f"Checkpoint saved at epoch {epoch + 1}, step {current_step}: {checkpoint_path}"
                            )
                            # print(f"Checkpoint saved at epoch {overall_epochs}, step {current_step}: {checkpoint_path}")

                print(
                    f"Chunk {chunk_idx + 1}/{num_chunks} finished. unloading dataset from memory..."
                )
                del dataset_full
                del dataset
                # del dataset
    ...
    torch.save(training_state, state_path)
    checkpoint_path = os.path.join(
        args.out_dir, f"checkpoint_epoch{epoch + 1}_step{current_step}.pt"
    )
    # checkpoint_path = os.path.join(args.out_dir, f"checkpoint_epoch{overall_epochs}_step{current_step}.pt")
    torch.save(model.state_dict(), checkpoint_path)
    # print(f"Final Checkpoint saved at epoch {overall_epochs}, step {current_step}: {checkpoint_path}")
    print(
        f"Final Checkpoint saved at epoch {epoch + 1}, step {current_step}: {checkpoint_path}"
    )
